async function getTodayWord() {
    try {
        const response = await fetch('http://backend:8000/current-word');
        if (!response.ok) {
...
async function validateWord(playerWord) {
    try {
        const response = await fetch(`http://backend:8000/validate/${playerWord}`)
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        };
        const data = await response.json()
...
async function checkWords(playerWord) {
    try {
        const response = await fetch(`http://backend:8000/check-word/${playerWord}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        };
        const data = await response.json();
